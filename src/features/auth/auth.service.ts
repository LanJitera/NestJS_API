...
export class OAuthService {
  ...
  async signUpAdmin(signUpDto: SignUpDto.AuthRegistrationAdminRequestDto) {
    try {
      const params = {
        ...
        password_confirmation: signUpDto.admin.password_confirmation,
  ...
  async verifyAdminConfirmationEmail(token: string) {
    const user = await this.adminRepo.findOneBy({ confirmation_token: token });
  ...
  async signUpUser(signUpDto: SignUpDto.AuthRegistrationUserRequestDto) {
    try {
      const params = {
        ...
        dateofbirth: signUpDto.user.dateofbirth,
      };
